...
// コンテキスト＝描画関連を司る機能
//ここ分からん、サイトの名前は”m_pImmediateContext”今の”g_pDeviceContext”との違いが判らない
ID3D11DeviceContext* g_pDeviceContext;
// スワップチェイン＝ダブルバッファ機能
IDXGISwapChain* g_pSwapChain;
//テクスチャー2D ＝ 深度、視点からの距離、Z軸
...
HRESULT D3D_Create(HWND hwnd)
{
	...
	swapChainDesc.BufferDesc.RefreshRate.Numerator = 60;				//一秒間に何回画面を更新するかの分母
	swapChainDesc.BufferDesc.RefreshRate.Denominator = 1;				//一秒間に何回画面を更新するかの分母
	//＝ようは60分の1になっているので1秒間で60回更新する
	...
	if (FAILED(hr))
	{
		return hr; // 上の関数呼び出しが失敗してないかifでチェック
	}
